            ...
            <div class="w-full lg:w-[45%] mb-4 flex justify-center align-center ">
                <div class="bg-white shadow-lg rounded-lg w-full">
                    <div class="bg-indigo-100 text-black p-4 rounded-t-lg" style="background: linear-gradient(to right, #0a22aa, #b62512); color: white; border: none;">
                        <strong style="color:white;">Fast Tag Recharge</strong>
                    </div>
                    <div class="p-6">
                        <form action="{{route('fastTagBillFetch')}}" method="post">
                            @csrf
                            <div class="mb-4">
                                <label for="customer-id" class="block text-sm font-medium">
                                    Vehicle Registration Number/Wallet Number *
                                </label>
                                <input type="text" name="customer-reg"
                                    class="form-input mt-1 w-full p-1.5 border border-gray-300"
                                    id="customer-id"
                                    placeholder="Vehicle Registration Number/Wallet Number">
                            </div>
                ...
                <div class="bg-white shadow-lg rounded-lg">
                    ...
                    <div class="p-6">
                        <p class="text-sm text-gray-500">
                            ...
                            <p><strong>Wallet Balance</strong>:        {{ $billData['additionalDetails'] ?? 'N/A' }}</p>
                            <p><strong>Vehicle Registration</strong>:      {{ $billData['registration'] ?? 'N/A' }}</p>
                            <p><strong>Maximum Permissible Recharge Amount</strong>:      {{ $billData['maximum'] ?? 'N/A' }}</p>
                        
                            <form action="{{route('fastTagRechargePay')}}" method="post">
                                @csrf
