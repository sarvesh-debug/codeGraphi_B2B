...
class creditCradBBPSController extends Controller
{
    public function index()
    {
         ...
         // die();
           if ($response->successful()) {
             ...
             return view('user.creditCardBBPS.index', compact('billers'));
           } else {
    ...
    public function creditCardBillFetch(Request $request){
        // return $request;
        // die();
       $ipAddress="103.254.205.164";  // Client's IP address
        // $macAddress = exec('getmac');
        // $macAddress = strtok($macAddress, ' ');
        $macAddress="00-15-5D-6D-D3-36";
        $consumerNumber=$request->input('cardNumber');
    ...
    public function creditCardRechargePay(Request $request)
{
...
private function updateCustomerBalance($mobile,$role,$externalRef)
{
    ...
    DB::table('getcommission')->insert([
        ...
        'sub_services' => 'Mobile Credit Card Bill',
        'externalRef' =>$externalRef,
        'amount'=>$txnAmount,
        'commission' => $retailerCommission,
        'tds' => $tds,
    ...
    if ($superDistributor && $superEarning > 0) {
     ...
     $apiBalance = ApiHelper::decreaseBalance(env('Business_Email'), $realAmount, 'Credit Card Bill');
//     dd([
