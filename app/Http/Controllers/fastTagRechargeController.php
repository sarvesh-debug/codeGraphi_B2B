...
class fastTagRechargeController extends Controller
{
    public function index()
    {
         ...
         // die();
           if ($response->successful()) {
             ...
             return view('user.fastTag.index', compact('billers'));
           } else {
    ...
    public function fastTagBillFetch(Request $request){
        // return $request;
        // die();
...
$macAddress="00-15-5D-6D-D3-36";
        $consumerNumber=$request->input('customer-reg');
        ...
        //  die();
         ...
         if ($data['statuscode'] === "TXN") {     
              session(['exID'=>$data['data']['enquiryReferenceId']] ?? null);
         $filteredData = [
             'geoCode' =>$geoCode,
             'mobile'=>$mobile,
             'billerId'=>$billerId,
             ...
             'additionalDetails' => $data['data']['AdditionalDetails'][2]["Value"] ?? null,
             'maximum' => $data['data']['AdditionalDetails'][0]["Value"] ?? null,
             'registration' => $data['data']["CustomerParamsDetails"][0]["Value"] ?? null,
         ];  
    ...
    public function fastTagRechargePay(Request $request)
{
...
private function updateCustomerBalance($mobile,$role,$externalRef)
{
    ...
    if ($superDistributor && $superEarning > 0) {
     ...
     $apiBalance = ApiHelper::decreaseBalance(env('Business_Email'), $realAmount, 'FastTag');
//     dd([
