    ...
    public function index()
    {
       ...
       // die();
         if ($response->successful()) {
           ...
           return view('user.landlinePost.index', compact('billers'));
         } else {
  ...
  $macAddress="00-15-5D-6D-D3-36";
   ...
   $externalRef = 'RPR' . date('Y') . '' . round(microtime(true) * 1000);
    ...
    //
        ...
        'inputParameters' => [
            'param1' => $$landlineNumber,
            'param2' => $$landlineNumber
        ],
        'deviceInfo' => [
            'mac' => $macAddress,
            'ip' => $ipAddress,
...
public function landlineBillPay(Request $request)
{
...
private function updateCustomerBalance($mobile,$role,$externalRef)
{
    ...
    DB::table('getcommission')->insert([
        ...
        'service' => 'LandlineBill', //Confirm with Sarvesh
        'sub_services' => 'LandlineBill',
        'externalRef' =>$externalRef,
        'amount'=>$txnAmount,
        'commission' => $retailerCommission,
        'tds' => $tds,
    ...
    if ($distributor && $distributorEarning > 0) {
        ...
        DB::table('dis_commission')->insert([
            ...
            'services' => 'LandlineBill', //confirm with Sarvesh
            'retailer_no' => $mobile,
    ...
    if ($superDistributor && $superEarning > 0) {
     ...
     $apiBalance = ApiHelper::decreaseBalance(env('Business_Email'), $realAmount, 'LandlineBill'); //Check with Sarvesh
//     dd([
