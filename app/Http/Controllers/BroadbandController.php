...
class BroadbandController extends Controller
{
    ...
    //    die();
         if ($response->successful()) {
           ...
           return view('user.broadbandRecharge.index', compact('billers'));
         } else {
...
public function broadbandRecharge(Request $request){
   //return $request;die();
  ...
  $macAddress="00-15-5D-6D-D3-36";
   ...
   $externalRef = 'TXN' . date('Y') . '' . round(microtime(true) * 1000);
    ...
    if ($data['statuscode'] === "TXN") {       
        ...
        return back()->with('error', 'Failed To Fetch.'.$message);
    }

}

public function mobileBillPay(Request $request)
{
...
private function updateCustomerBalance($mobile,$role,$externalRef)
{
    ...
    DB::table('getcommission')->insert([
        ...
        'service' => 'Mobile_RechargePostPaid',
        'sub_services' => 'Mobile RechargePostPaid',
        'externalRef' =>$externalRef,
        'amount'=>$txnAmount,
        'commission' => $retailerCommission,
        'tds' => $tds,
    ...
    if ($distributor && $distributorEarning > 0) {
        ...
        DB::table('dis_commission')->insert([
            ...
            'services' => 'MobileRechargePostPaid',
            'retailer_no' => $mobile,
    ...
    if ($superDistributor && $superEarning > 0) {
     ...
     $apiBalance = ApiHelper::decreaseBalance(env('Business_Email'), $realAmount, 'MobileRechargePostPaid');
//     dd([
